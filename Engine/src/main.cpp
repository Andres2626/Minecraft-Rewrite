...
static bool close = false; /* close flag */
static bool last_mouse_right = false; /* mouse left button flag */
static bool last_mouse_left = false; /* mouse right button flag */
static Window *win;
static Player *steve;
static Shader *sh_chunk;
static Shader *sh_sel;
static Selector* py_sel;
static Level *lev;
static Timer *timer;
...
void PlayerPick()
{
	...
	if (hit) {
		...
		py_sel->SetHit(ret);
		py_sel->Render(steve->cam, sh_sel, timer->ElapsedMillis());

		/* Get mouse button status */
		bool left = glfwGetMouseButton(win->GetWindow(), GLFW_MOUSE_BUTTON_1) == GLFW_PRESS;
		bool right = glfwGetMouseButton(win->GetWindow(), GLFW_MOUSE_BUTTON_2) == GLFW_PRESS;
...
{
	steve->tick(win->GetWindow());
}

void OnTick() 
{
#if _DEBUG
	MC_INFO("fps: %i ups: %i cups: %i", fps, ups, lev->GetUpdates());
#else
	printf("%i fps, %i\n", fps, lev->GetUpdates());
#endif
}
...
int main(int argc, char** argv)
{
	...
	pr.y = WIN_Y;
	pr.title = "Game";
	pr.context = { CONTEXT_ANY_PROFILE, 0, 0, false };
	pr.cursor = { false };
	win = new Window(pr);
	...
	glfwSetWindowIcon(win->GetWindow(), 1, icon);
	stbi_image_free(icon[0].pixels);
	 ...
	 * -- Clear window
	 * -- Obtain now time
	 * -- Update every 50ms (1/60s which is the tick value)
	 * -- Render game
	 * -- Every second it does the OnTick() process
	 * -- OnEvent process
	 */
	timer = new Timer();
	float ti = 0.0f;
	float update_timer = timer->ElapsedMillis();
	float update_tick = 1000.0f / TPS;
	Timestep step(timer->ElapsedMillis());
	while (!close) {
		win->Clear();
		...
		if (now - ti > 1000.0f) {
			...
			/* Restart CUPS (chunk updates per second) */
			lev->RestartUpdates();
		}
		OnEvent(ev);
	}
